  ...
  livox_interfaces__msg__CustomPoint__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    livox_interfaces__msg__CustomPoint * data =
      (livox_interfaces__msg__CustomPoint *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!livox_interfaces__msg__CustomPoint__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          livox_interfaces__msg__CustomPoint__fini(&data[i]);
        }
        free(data);
        return false;
      }
