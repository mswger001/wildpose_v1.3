  ...
  livox_interfaces__msg__CustomMsg__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    livox_interfaces__msg__CustomMsg * data =
      (livox_interfaces__msg__CustomMsg *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!livox_interfaces__msg__CustomMsg__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          livox_interfaces__msg__CustomMsg__fini(&data[i]);
        }
        free(data);
        return false;
      }
